...
import { FaHeadphones, FaTimes, FaHeart, FaDiscord } from 'react-icons/fa';

const WelcomeModal: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  
  // KİLİT MEKANİZMASI: Timer'ın sadece bir kez kurulmasını sağlar
  const timerSet = useRef(false); 

  useEffect(() => {
    if (typeof window === 'undefined') return;
    const hasSeen = localStorage.getItem('merky_welcome_seen');
    
    // Eğer daha önce görülmediyse VE timer henüz kurulmadıysa
    if (!hasSeen && !timerSet.current) {
      
      timerSet.current = true; // Kilidi kapat, bir daha girmesin
  ...
  const handleClose = () => {
    localStorage.setItem('merky_welcome_seen', 'true');
    setIsOpen(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      {/* ... Kodun geri kalanı aynı ... */}
        {/* Sadece üst kısımdaki mantığı değiştirmen yeterli, tasarım kodları aynı */}
        <div 
      ...
      <div 
        className="relative w-full max-w-md bg-[#080808] border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden animate-[reveal-up_0.6s_ease-out_forwards]"
        ...
        <div className="relative p-8 flex flex-col items-center text-center gap-6">
           ...
           <div className="space-y-3">
             ...
             <p className="text-sm text-gray-400 leading-relaxed font-light">
               Experience high-fidelity lo-fi beats curated for deep focus, relaxation, and coding sessions. Powered by Discord SDK.
             </p>
           </div>

           <div className="grid grid-cols-2 gap-3 w-full">
              <div className="flex items-center justify-center gap-2 p-3 bg-white/5 rounded-xl border border-white/5">
                 <FaHeadphones className="text-orange-500 text-sm" />
                 <span className="text-xs text-gray-300 font-medium">Hi-Fi Audio</span>
              </div>
              <div className="flex items-center justify-center gap-2 p-3 bg-white/5 rounded-xl border border-white/5">
